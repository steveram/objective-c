...
/**
 @brief PubNub service reported that provided location key path can't be used.

 @since <#version number#>
 */
static NSInteger const kPNInvalidRemoteObjectDataPathError = 118;

/**
 @brief PubNub service reported that data at provided location key can't be updated.

 @since <#version number#>
 */
static NSInteger const kPNRemoteObjectDataUpdateError = 119;

/**
 @brief PubNub service reported that data at provided location key can't be deleted.

 @since <#version number#>
 */
static NSInteger const kPNRemoteObjectDataDeleteError = 120;

/**
 @brief PubNub service reported that data at provided location key can't be replaced.

 @since <#version number#>
 */
static NSInteger const kPNRemoteObjectDataReplaceError = 121;

/**
 @brief PubNub service reported that tried developer tried to replace/push to remote object root with
        non-collection object.

 @since <#version number#>
 */
static NSInteger const kPNRemoteObjectRootNodeError = 122;
...
/**
 ...
 @since 3.7.0
 */
static NSInteger const kPNEmptyChannelGroupError = 123;
...
/**
 ...
 @since 3.7.0
 */
static NSInteger const kPNEmptyChannelGroupSizeExceededError = 124;
...
/**
 ...
 @since 3.7.2
 */
static NSInteger const kPNChannelGroupNotEnabledError = 125;

/**
 @brief PubNub service reported that developer tried to use DataSync feature which is not enabled
        for account.

 @since <#version number#>
 */
static NSInteger const kPNDataSyncNotEnabledError = 126;

// PubNub service reported that there is no authorization key specified and resource not available w/o it
static NSInteger const kPNAPIUnauthorizedAccessError = 127;

// PubNub service reported that wrong authorization has been used for request
static NSInteger const kPNAPIAccessForbiddenError = 128;

/**
 \b PubNub service reported that requested API not enabled or not available.
 */
static NSInteger const kPNAPINotAvailableOrNotEnabledError = 129;

/**
 Scheduled request can't be processed w/o initial API calling based on object data.
 */
static NSInteger const kPNRequestCantBeProcessedWithOutRescheduleError = 130;
...
static NSInteger const kPNMessageObjectError = 138;

// Developer tried to submit message w/o text to PubNub service
static NSInteger const kPNMessageHasNoContentError = 139;

// Developer tried to submit message w/o target channel to PubNub service
static NSInteger const kPNMessageHasNoChannelError = 140;

/**
 Developer tried to push state w/ invalid content (used keys with forbidden prefix or unsupported values).
 */
static NSInteger const kPNInvalidStatePayloadError = 141;

/**
 Developer tried to push state for channel on which client not subscribed at this moment.
 */
static NSInteger const kPNCantUpdateStateForNotSubscribedChannelsError = 142;

/**
 @brief Developer tried to use restricted characters for remote object identifier.
 
 @since <#version number#>
 */
static NSInteger const kPNIncompatibleRemoteObjectNameError = 143;

/**
 @brief Developer tried to use location key-path which doesn't conform to existing requirements
        described in API documentation.
 
 @since <#version number#>
 */
static NSInteger const kPNIncompatibleRemoteObjectDataLocationError = 144;

// Developer tried to use APNS API w/o enabling push notifications support on admin.punub.com
static NSInteger const kPNPushNotificationsNotEnabledError = 145;

/**
 Developer tried to use PAM API w/o specifying \a 'secret' key.
 */
static NSInteger const kPNSecretKeyNotSpecifiedError = 146;

// Developer tried to use empty device push notification to enable push notification on specified channel
static NSInteger const kPNDevicePushTokenIsEmptyError = 147;

// Developer tried to to obtain history in the same time when it is disabled on portal.
static NSInteger const kPNStorageNotEnabledError = 148;
