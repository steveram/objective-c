...
/**
 @brief Filter provided list of channels and return only remote object synchronization feed 
        channels.
 
 @param channelsList List of channels which should be sorted.
 
 @return Empty array in case if there is no remote object synchronization feed channels.
...
- (void)leaveChannels:(NSArray *)channels byUserRequest:(BOOL)isLeavingByUserRequest {
    ...
    if ([channelsForUnsubscribe count] > 0) {
        ...
        PNLeaveRequest *request = [PNLeaveRequest leaveRequestForChannels:[channelsForUnsubscribe copy]
        ...
        if (isLeavingByUserRequest) {
            ...
            [self.messagingDelegate messagingChannel:self willUnsubscribeFrom:channelsForUnsubscribe sequenced:NO];
        }
                ...
                if ((isPresenceModification && indirectionalPresenceModification) || isChangingPresenceOnSubscribedChannels ||
                    ...
                    [self.messagingDelegate clientStateInformation:^(NSDictionary *fullClientState) {
                        ...
                        [self pn_dispatchBlock:^{
                            ...
                            dispatch_block_t reportCompletionBlock = ^{
                                ...
                                if ([restOfChannels count]) {
                                
                                    [self.messagingDelegate messagingChannel:self didSubscribeOn:[restOfChannels allObjects]
                                                                   sequenced:isPresenceModification
                                                             withClientState:clientStateForRequest];
                                }
                                
                                if ([synchronizationChannels count]) {
                                    
                                    NSArray *baseSynchronizationChannels = [PNSynchronizationChannel baseSynchronizationChannelsFromList:synchronizationChannels];
                                    [self.messagingDelegate messagingChannel:self
                                      didSubscribeOnRemoteObjectsChangesFeed:baseSynchronizationChannels
                                                                   sequenced:isPresenceModification];
                                }
                            };
                ...
                dispatch_block_t requestCompletionBlock = ^{
                    ...
                    [self pn_dispatchBlock:^{
                        ...
                        if (hasValidSetOfChannels) {
                            ...
                            if ([[subscribeRequest.channelsForSubscription lastObject] isTimeTokenChangeLocked] &&
                                ![subscribeRequest isInitialSubscription]) {
    ...
    [self pn_dispatchBlock:^{
        ...
        if ([currentlySubscribedChannels intersectsSet:channelsWithPresence]) {
            ...
            if ([restOfChannels count]) {
                ...
                [self.messagingDelegate messagingChannel:self didUnsubscribeFrom:[restOfChannels allObjects]
                                               sequenced:[synchronizationChannels count]];
            }
            
            if ([synchronizationChannels count]) {
                
                NSArray *baseSynchronizationChannels = [PNSynchronizationChannel baseSynchronizationChannelsFromList:synchronizationChannels];
                [self.messagingDelegate messagingChannel:self didUnsubscribeFromRemoteObjectsChangesFeed:baseSynchronizationChannels
                                               sequenced:NO];
            }
    ...
    if (request.isSendingByUserRequest) {
        ...
        if (![self hasRequestsWithClass:[PNSubscribeRequest class]]) {
            ...
            if ([channels count]) {
                ...
                [self.messagingDelegate messagingChannel:self didUnsubscribeFrom:channels
                                               sequenced:[synchronizationChannels count]];
            }
            ...
            if ([synchronizationChannels count]) {
                ...
                NSArray *baseSynchronizationChannels = [PNSynchronizationChannel baseSynchronizationChannelsFromList:synchronizationChannels];
                [self.messagingDelegate messagingChannel:self
              didUnsubscribeFromRemoteObjectsChangesFeed:baseSynchronizationChannels
                                               sequenced:NO];
            }
        }
    }
    ...
    if ([parsedData isKindOfClass:[PNError class]] ||
        ...
        if ([events.events count] > 0) {
            ...
            [events.events enumerateObjectsUsingBlock:^(id event, NSUInteger eventIdx, BOOL *eventsEnumeratorStop) {
                ...
                else if ([event isKindOfClass:[PNDataSynchronizationEvent class]]) {
            ...
            if ([channels count]) {
                ...
                [self.messagingDelegate messagingChannel:self didFailSubscribeOn:channels withError:error
                                               sequenced:([subscriptionRequest.channelsForPresenceEnabling count] ||
                                                          ...
                                                          [synchronizationChannels count])];
            }
            
            if ([synchronizationChannels count]) {
                
                [self.messagingDelegate messagingChannel:self
              didFailSubscribeOnRemoteObjectsChangesFeed:[PNSynchronizationChannel baseSynchronizationChannelsFromList:synchronizationChannels]
                                               withError:error sequenced:([subscriptionRequest.channelsForPresenceEnabling count] ||
                                                                          [subscriptionRequest.channelsForPresenceDisabling count])];
            }
        }
    }
    ...
    [channelsList enumerateObjectsUsingBlock:^(PNChannel *channel, NSUInteger channelIdx, BOOL *channelEnumeratorStop) {
        ...
        if (!fetchPresenceChannelsOnly) {
            ...
            if(!shouldExcludeSynchronizationChannels ||
               ![PNSynchronizationChannel isObjectSynchronizationChannel:channel.name]) {
...
- (NSArray *)channelsWithOutPresenceFromList:(NSArray *)channelsList
    ...
    if (shouldExcludeSynchronizationChannels) {
        ...
        predicateFormat = @"(isPresenceObserver = NO) AND !(isForDataSynchronization = YES)";
    }
...
- (NSArray *)channelsWithPresenceFromList:(NSArray *)channelsList
    ...
    if (shouldExcludeSynchronizationChannels) {
        ...
        predicateFormat = @"(isPresenceObserver = YES) AND !(isForDataSynchronization = YES)";
    }
...
- (NSArray *)channelsForDataSynchronizationFromList:(NSArray *)channelsList {
    ...
    NSPredicate *filterPredicate = [NSPredicate predicateWithFormat:@"isForDataSynchronization = YES"];
                    ...
                    if ([request isKindOfClass:[PNSubscribeRequest class]]) {
                        ...
                        if ([channelsForSubscription count] || [PNBitwiseHelper is:self.messagingState containsBit:PNMessagingChannelRestoringSubscription]) {
                            ...
                            if ([PNBitwiseHelper is:self.messagingState containsBit:PNMessagingChannelRestoringSubscription]) {
                                ...
                                if ([synchronizationChannels count]) {
                                    ...
                                    [self.messagingDelegate messagingChannel:self
                                      didSubscribeOnRemoteObjectsChangesFeed:[PNSynchronizationChannel baseSynchronizationChannelsFromList:synchronizationChannels]
                                                                   sequenced:isInSequence];
                                }
                            }
                        }
                        ...
                        if ([existingChannelsSet count]) {
                            ...
                            [PNLogger logCommunicationChannelInfoMessageFrom:self withParametersFromBlock:^NSArray * {
                                ...
                                return @[PNLoggerSymbols.connectionChannel.subscribe.unsubscribedFromSetOfChannels, (self.name ? self.name : self),
                                        (allChannelsFromLeaveRequest ? allChannelsFromLeaveRequest : [NSNull null]), @(self.messagingState)];
                            }];
                            ...
                            if ([synchronizationChannels count]) {
                                ...
                                [self.messagingDelegate messagingChannel:self
                              didUnsubscribeFromRemoteObjectsChangesFeed:[PNSynchronizationChannel baseSynchronizationChannelsFromList:synchronizationChannels]
                                                               sequenced:([subscribeRequest.channelsForPresenceEnabling count] ||
                                                                          [subscribeRequest.channelsForPresenceDisabling count])];
                            }
                        }
                        ...
                        [PNLogger logCommunicationChannelInfoMessageFrom:self withParametersFromBlock:^NSArray * {
                            ...
                            return @[PNLoggerSymbols.connectionChannel.subscribe.leaved, (self.name ? self.name : self),
                                    (leaveRequest.channels ? leaveRequest.channels : [NSNull null]), @(self.messagingState)];
                        }];
